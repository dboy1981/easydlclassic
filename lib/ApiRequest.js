...
ApiRequest.prototype = {
  get_accesstoken: async () => {
    var token = null;
    try{
      token = await OAuth.getToken(this.key, this.secrect);
    }catch(err){
      throw err;
    }
    return token;
  ...
  post: (url, data, repeat = 1) => {
    var promise = new Promise(async (resolve, reject) => {
      var token = null;
      try{
        token = await this.get_accesstoken();
      }catch(err){
        return reject(err);
      }
      ...
      var callback = (err, response, body) => {
        ...
        if(info.error_code == 110 && repeat == 1){
          OAuth.refreshToken(this.key, this.secrect);
          try{
            resolve(await this.post(url, data, 0));
          }catch(err){
            reject(err);
          }
          return;
